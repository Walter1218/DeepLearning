...
class NeuralNetwork_V1(object):
	...
	def train(self, inputs_list, label_list):
		...
		hhidden_errors = np.dot(self.weights_hidden_to_output.T, error)# errors propagated to the hidden layer
		hhidden_grad = self.hidden_outputs_1 * (1 - self.hidden_outputs_1)# hidden layer gradients
